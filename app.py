...
# CT models (trained separately, as in your code)
...
# -------------------------------------------------------------
# Class Labels (MUST match training folder order / class_indices)
# -------------------------------------------------------------
# MRI Level-1
...
# CT tasks
classes_ct_stroke = ['Haemorrhagic', 'Ischemic', 'Normal']   # stroke/
classes_ct_tumor  = ['Healthy', 'tumor']                    # Tumor/Healthy + Tumor/tumor

# -------------------------------------------------------------
# Recommendations (you can expand)
# -------------------------------------------------------------
recommendations = {
    "stroke": {
        "Haemorrhagic": {
        ...
        "Ischemic": {
            ...
            "Medications": [
                "Antiplatelets (e.g., Aspirin/Clopidogrel as prescribed)",
    ...
    "ct_tumor": {
        "tumor": {
            ...
            "Medications": [
                ...
                "Definitive treatment depends on clinical evaluation"
            ]
        },
        "Healthy": {
            "Precautions": [
                "Maintain good lifestyle habits and routine health checks."
            ],
            "Medications": ["No medication required."]
        }
    }
...
# For CT, user must choose task because you trained 2 separate models
ct_task = None
if modality == "CT":
...
# -------------------------------------------------------------
# CT Prediction (two separate tasks)
# -------------------------------------------------------------
    ...
    if st.button("🔍 Predict"):
        with st.spinner("Analyzing..."):
            ...
            if modality == "MRI":
                ...
                st.info("⚕️ Please consult a qualified clinician for confirmation and treatment decisions.")
                ...
                if ct_task == "Stroke classification":
                    ...
                    for p in recommendations["stroke"].get(stroke_label, {}).get("Precautions", []):
                        st.write(f"- {p}")

                    st.subheader("💊 Medications")
                    for m in recommendations["stroke"].get(stroke_label, {}).get("Medications", []):
                        st.write(f"- {m}")
                    ...
                    st.subheader("🩺 Precautions")
                    for p in recommendations["ct_tumor"].get(tumor_label, {}).get("Precautions", []):
                        st.write(f"- {p}")

                    st.subheader("💊 Medications")
                    for m in recommendations["ct_tumor"].get(tumor_label, {}).get("Medications", []):
                        st.write(f"- {m}")
